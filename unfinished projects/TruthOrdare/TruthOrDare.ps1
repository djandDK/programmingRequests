...
if (Test-Path $path -PathType Leaf) {
    ...
    $regexMatches.Matches | Foreach-Object {
        $questionObject | Add-Member -NotePropertyName $($_.Groups['number'].Value + $_.Groups['letter'].Value.ToLower()) -NotePropertyValue $_.Groups['text'].Value
    }
    do {
        ...
        if ($regexMatch.Matches.Groups[1].Success -eq 'True' -and $regexMatch.Matches.Groups[2].Success -eq 'True') {
            $numberLetter = $($regexMatch.Matches.Groups[1].Value + $regexMatch.Matches.Groups[2].Value.ToLower())
            try {
